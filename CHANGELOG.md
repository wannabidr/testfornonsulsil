...
## v1.2 - 핀 연결 테스트 기능 추가 (2025-12-02)

### 테스트 시퀀스 개선
- **이전**: 단일 메시지 표시
- **현재**: 4단계 자동 순환 테스트
  1. 16진수 패턴 (데이터 비트 테스트)
  2. 알파벳 패턴 (문자 표시 테스트)
  3. 숫자 반복 패턴 (타이밍 테스트)
  4. 성공 메시지 (최종 확인)

### 주요 변경 사항
- 각 테스트 패턴을 2초 간격으로 자동 전환
- 1초 타이머 추가 (100MHz 클럭 기준)
- 6개 상태로 확장된 FSM
- 핀 연결 문제 진단 가능

### 추가 파일
- `PIN_TEST_GUIDE.md`: 상세한 핀 테스트 및 문제 해결 가이드
- 데이터 비트 매핑 테이블
- 하드웨어 체크리스트
- 문제 해결 플로우차트

### 장점
- 데이터 버스 각 비트별 연결 상태 확인 가능
- 제어 신호(RS, RW, E) 동작 확인
- 타이밍 문제 조기 발견
- 사용자 개입 없이 자동 테스트
...
## v1.1 - 클럭 주파수 최적화 (2025-12-02)

### 클럭 주파수 변경
- **변경**: 1kHz → 1MHz (1000배 향상)
- **분주비**: 100000분주 → 100분주
- **tick 주기**: 1ms → 1μs
- **이유**: 더 빠른 LCD 업데이트, HD44780 표준 타이밍 정확도 향상

### 타이밍 상수 조정
- DELAY_15MS: 15 → 15000 (15000μs)
- DELAY_5MS: 5 → 5000 (5000μs)
- DELAY_2MS: 2 → 2000 (2000μs)
- DELAY_100US: 새로 추가 (100μs)
- DELAY_50US: 새로 추가 (50μs)

### 성능 개선
- 전체 화면 업데이트: 약 34ms → 약 1.9ms (17배 빠름)
- 문자당 쓰기 시간: 1ms → 50μs (20배 빠름)

---

## v1.0 - 초기 버전 (Verilog-2001 표준 준수)
